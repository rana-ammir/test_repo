...
(function() {
  $(function() {
    $('#user_organization_name').bind('railsAutocomplete.select', function(event, data) {
      $('#organization_id').val(data.item.key);
      return $('#user_division_id').data({
        organization_id: data.item.key
      });
    });
    $('#team_organization_name').bind('railsAutocomplete.select', function(event, data) {
      return $('#team_organization_id').val(data.item.key);
    });
    return $('.division-select').on('change', function(e) {
      var division_id;
